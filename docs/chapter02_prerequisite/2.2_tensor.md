...
这里不详细介绍，用到了再查官方文档。
### 改变形状
用`view()`来改变`Tensor`的形状：# 这种修改似乎是把元素“挨个挨个放过去”，按照低维到高维，下标递增的顺序；**共享内存**
``` python
y = x.view(15)
...
1. Tensor的创建
   1. 全新的Tensor
      1. 各种创建Tensor的函数`torch.tensor() torch.rand() torch.zeros() torch.new_ones() `
      2. clone `y = x.clone().view(25, 1)`
2. Tensor信息获取
   1. 形状`x.size() x.shape`
3. 运算
   1. 加法
      1. 直接相加`x + y`，经简单测试，似乎是全新的Tensor
      2. `torch.add(x, y)`
      3. `torch.add(x, y, out=result)`
      4. inplace `y.add_(x)`
4. 索引 `x = torch.rand(5, 5)` `y = x[0, :]` 共享内存
5. 改变形状`y = x.view(25, 1)` 共享内存
6. 转标量 `x.item()`（单数字Tensor的操作）
7. 广播机制（计算时升维）
8. 和numpy的转换
   1. 共享内存
      1. `x = torch.ones(5)` `a = x.numpy()`
      2. `a = np.ones(5)` `x = torch.from_numpy(a)`
   2. 新建
      1. `y = torch.tensor(a)`
9. GPU
   1. 查看device `torch.device("cuda")`
   2. 转换device `x = torch.Tensor(***)` `x.to("CPU")` `x.to(CUDA)`（内存位置并未变）
